...
int compar(const void *p1, const void *p2) {
    const char *rec1 = *(char **) p1;
    const char *rec2 = *(char **) p2;
...
//                printf("BEFORE: INDICES[j] = %d, INDICES[j+1] = %d\n", indices[j], indices[j+1]);
                temp = indices[j];
                indices[j] = indices[j + 1];
                indices[j + 1] = temp;
//                printf("j = %d", j);
//                printf("I'M HERE\n");
//                printf("t2D[j] = %s, t2D[j+1] = %s\n", t2D[j], t2D[j+1]);
//                printf("AFTER: INDICES[j] = %d, INDICES[j+1] = %d\n\n", indices[j], indices[j+1]);
            }
        }
    }
//
